<?php
/** * KD·boT * Copyright (c) 2015~2017 NULLMIX&KD·Lab All Rights Reserved. *  */
set_time_limit(300);
...
/*get postlock list*/
//$postlock=json_decode(file_get_contents(SYSTEM_ROOT . '/db/postlock.json') , 1);//即将启用
/*get replacelist*/
    ...
    if ($autoget == '') {
        ...
        switch ($task["post_type"]) {
            case "ajax":
                ...
                fp(SYSTEM_ROOT . '/db/log.csv', "a", json_encode(array(
                    time() ,
                    $a,
                    $task["name"],
                    $text
